/**
 * Live-database smoke test for the Postgres migration.
 *
 * Replicates the exact login + posting SQL paths (via src/db/queries.ts) a
 * browser session would take, against the real DATABASE_URL in .env.local.
 * Creates a throwaway user/session/post and deletes them all afterwards.
 *
...
// Load DATABASE_URL from .env.local (Node doesn't read it automatically).
...
async function main(): Promise<boolean> {
  ...
  if (!session || session.userId !== userId) throw new Error('session round-trip failed');
